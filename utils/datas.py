...
# 得到归一化后特定大小的图片：先进行剪切，再重新调整大小，最后归一化
...
class celebA():
    ...
    # 展示样本
    def data2fig(self, samples, size=4):
...
"""
    当flag为"mlp"时，简单地返回一批数据
    当flag为"conv"时，返回reshape的一批数据
"""
class mnist():
